...
function loadFromLocalStorage() {
    ...
    try {
        ...
        if (storedAdLinks) {
            ...
            console.log("Created default ad links");
        }
...
function displayAdLinks() {
    ...
    // Get active ad indices from localStorage (now supports multiple)
    let activeAdIndices = [];
    try {
        const activeAdsData = localStorage.getItem('activeAdIndices');
        if (activeAdsData) {
            activeAdIndices = JSON.parse(activeAdsData);
            if (!Array.isArray(activeAdIndices)) {
                // Convert old format (single index) to new format (array)
                const oldIndex = parseInt(localStorage.getItem('lastDisplayedAdIndex'));
                if (!isNaN(oldIndex) && oldIndex >= 0 && oldIndex < adLinks.length) {
                    activeAdIndices = [oldIndex];
                } else {
                    activeAdIndices = [];
                }
            }
        ...
        // If no active ads, default to first one
        if (activeAdIndices.length === 0 && adLinks.length > 0) {
            activeAdIndices = [0];
        }
        ...
        activeAdIndices = adLinks.length > 0 ? [0] : [];
        localStorage.setItem('activeAdIndices', JSON.stringify(activeAdIndices));
    }
...
async function addAdLink() {
    ...
    if (newLink) {
        ...
        try {
            ...
            alert("Error saving ad link. Please try again.");
        }
    }
}
